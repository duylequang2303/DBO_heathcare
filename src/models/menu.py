...
class Menu:
    ...
    def decode(
        ...
        if len(food_ids) != len(portions_g):
            raise ValueError(
                "food_ids and portions_g must have the same length"
            )
        ...
        if sum(counts) != len(food_ids):
            raise ValueError(
                "sum(meal_counts) must equal len(food_ids)"
            )
        ...
        for meal_type, count in zip(
            ...
            for fid, portion in zip(
                food_ids[offset:offset + count],
                portions_g[offset:offset + count],
            ):
                row = food_map[fid]
    ...
    def __repr__(self) -> str:
        ...
        for meal_type, meal in self.meals.items():
            names = [
                f"{item.food_name}({item.portion_g:.0f}g)"
                for item in meal.items
            ]
